...
class Movies extends Component {
  ...
  like: isLike
      ...
      if (isLike){
      // Here, we want to save a movie in our db
      fetch('http://10.2.3.118:3000/mymovies', {
 method: 'POST',
 ...
 body: `title=${this.props.movieName}&overview=${this.props.movieDesc}&poster_path=${this.props.movieImg}&idMovieDB=${this.props.idMovie}`
    ...
    fetch(`http://10.2.3.118:3000/mymovies/${this.props.idMovie}`, {
    method: 'DELETE'})
 .catch((error) => {
    console.error(error);
  });
...
<div style={{marginBottom:30}}>
      <Card className="Card">
        ...
        <CardTitle>{this.props.moviesName}</CardTitle>
